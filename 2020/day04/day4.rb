...
result = data.select do |passport_data|
...
class StrictPassport
  ...
  def valid?
    has_required_fields? && all_fields_valid?
  end
  ...
  def all_fields_valid?
    (1920..2002).cover?(data["byr"].to_i) &&
      ...
      valid_height?(data["hgt"]) &&
      valid_hair_color?(data["hcl"]) &&
      valid_eye_color?(data["ecl"]) &&
      valid_pid?(data["pid"])
  end

  def valid_height?(height)
    return false unless height.include?("cm") || height.include?("in")

    h = height.match(/\d+/).to_s.to_i
...
result = data.select do |passport_data|
  passport = StrictPassport.new(passport_data)
  passport.valid?
end
