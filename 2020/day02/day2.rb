...
valid = data.select do |line|
  min, max, char, str = line.scan(/(\d+)-(\d+)\s(\w):\s([a-z]+)/).flatten
...
valid = data.select do |line|
  count = 0
  pos1, pos2, char, str = line.scan(/(\d+)-(\d+)\s(\w):\s([a-z]+)/).flatten
  pos1 = pos1.to_i - 1
  pos2 = pos2.to_i - 1
  count += 1 if str[pos1] == char
  count += 1 if str[pos2] == char
  count == 1
end
