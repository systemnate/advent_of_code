...
set_default_hash = Hash.new { |h, k| h[k] = Set.new }

parsed_data = data.each_with_object(set_default_hash) do |rule, hash|
  initial_color = rule.match(/^([a-z\s]+) bags contain/)[1]
  contained_colors = rule.scan(/\d+\s+([a-z\s]+)\sbags?/).flatten

  hash[initial_color] += contained_colors
end
...
parsed_data.keys.each do |color|
  next if color == "shiny gold"
  dfs(parsed_data, color)
end
