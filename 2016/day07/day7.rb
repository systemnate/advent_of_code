...
class IpV7
  attr_reader :regular, :hypernet

  def self.parse(string)
    parsed = string.scan(/\[(\w+)\]|(\w+)/)

    regular, hypernet = parsed.partition { _1.first.nil? }
    regular = regular.map(&:compact).flatten
    hypernet = hypernet.map(&:compact).flatten

    new(regular, hypernet)
  end

  def initialize(regular, hypernet)
    @regular = regular
    @hypernet = hypernet
  end

  def supports_tls?
    regular.any? do |string|
      abba?(string)
    end && hypernet.none? do |string|
      abba?(string)
    end
  end

  def supports_ssl?
    regular_abas = all_abas(regular)
    hypernet_abas = all_abas(hypernet)

    possible_babs = regular_abas.map { aba_to_bab(_1) }

    hypernet_abas.any? { |str| possible_babs.include?(str) }
  end

  private

  def abba?(str)
    str.chars.each_cons(4) do |a, b, c, d|
      return true if a != b && a == d && b == c
    end
    false
  end

  def all_abas(subnets)
    subnets.each_with_object([]) do |str, abas|
      str.chars.each_cons(3) do |a, b, c|
        if a != b && a == c
          abas << [a, b, a].join
        end
      end
...
data = data.map { IpV7.parse(_1) }

puts data.select(&:supports_tls?).count
puts data.select(&:supports_ssl?).count
__END__
