...
class Room
  attr_reader :value, :sector_id, :checksum, :line

  def self.parse(line)
    parsed_line = line.split("-")
    last = parsed_line.pop
    sector_id, checksum = last.scan(/^(\d+)\[(\w+)\]$/).flatten
    ...
    new(line, result, sector_id.to_i, checksum)
  end

  def initialize(line, value, sector_id, checksum)
    @line = line
    @value = value
    @sector_id = sector_id
    @checksum = checksum
  ...
  def decrypt
    original = ('a'..'z').to_a.join
    rotated = ('a'..'z').to_a.rotate(sector_id).join
    line.tr(original, rotated).scan(/\D+/).first.split("-").join(" ")
  end
end

data = DATA.readlines.map(&:chomp)

puts data.map { |d| Room.parse(d) }.select(&:real?).map(&:sector_id).sum
...
north_pole = data
  ...
  .select { |k, v| k == "northpole object storage" }
  .flatten

puts north_pole.last
__END__
