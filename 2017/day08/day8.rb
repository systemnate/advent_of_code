...
instructions.each do |instruction|
  reg, com, amt, _if, con_reg, con, con_amt = instruction.split(" ")

  value_in_if_register = registers[con_reg]

  res = eval("#{value_in_if_register} #{con} #{con_amt}")

  if res
    if com == "inc"
      registers[reg] += amt.to_i
    else
      registers[reg] -= amt.to_i
    end
    max_value = registers[reg] if registers[reg] > max_value
  end
end
