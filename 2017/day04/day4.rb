...
data = DATA.readlines.map(&:chomp).map(&:split)

# part 1
result = data.each_with_object([]) do |passphrase, array|
  set = Set.new
  passphrase.each { |phrase| set << phrase }

  if set.size == passphrase.size
    array << passphrase
  end
end
...
class AnagramSet < Set
  def <<(item)
    included_in_set = false

    self.each do |phrase|
      if anagram?(phrase, item)
        included_in_set = true
      end
    end

    unless included_in_set
      super << item
    end
  end
  ...
  def anagram?(str1, str2)
    clean_str1 = str1.downcase.gsub(/\s+/, '')
    clean_str2 = str2.downcase.gsub(/\s+/, '')
...
result = data.each_with_object([]) do |passphrase, array|
  set = AnagramSet.new
  passphrase.each do |phrase|
    chars = phrase.chars
    permutations = chars.permutation.to_a.uniq.map(&:join)
