...
Direction = Struct.new(:command, :start_coord, :end_coord)

light_grid = Array.new(1_000) { Array.new(1_000, false) }

data = DATA.readlines.map(&:chomp).map do |direction|
  command = direction.scan(/toggle|on|off/).first
  start_coord, end_coord = direction.scan(/(\d+,\d+)/).map do |coord|
    coord.first.split(",").map(&:to_i)
  end
  Direction.new(command, start_coord, end_coord)
end

def set_lights(array, top_left, bottom_right, command)
...
data.each do |dir|
  set_lights(light_grid, dir.start_coord, dir.end_coord, dir.command)
end
